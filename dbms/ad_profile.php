      ...
      <div class="row">
        ...
        <div class="row">
          ...
          if(isset($_POST["add"])){
             ...
             $tm=md5(time());
			 $fnm=$_FILES["img"]["name"];
			 $dst="./ad_images/".$tm.$fnm;
			 $dst1="ad_images/".$tm.$fnm;	 
			 move_uploaded_file($_FILES["img"]["tmp_name"],$dst);
             $date = date("Y-m-d");
             $qr = "INSERT INTO advertisement VALUES ('','$dst1','$name','$disc','$_SESSION[id]')";
             mysqli_query($link,$qr);
             ?><script>alert("advertisement successfully added.");</script><?php
             ?><script>window.location="ad_profile.php";</script><?php
           }?>
        </div>
