...
<html>
  ...
  <div style="padding:0px 0px" class="container-fluid">
    <div class="row">
      <div class="col-sm-2">
        <img style="margin-bottom: 15px;margin-left:60px;box-shadow: 5px 0px 15px rgba(0,0,0, .5);" src="picr.svg"
	...
	?>
          ...
          if(isset($_POST["buy"])){
              if($_SERVER["REQUEST_METHOD"] == "POST"){
                ...
                if($result["quantity"]>=$qty){
                  ...
                  $date = date("Y-m-d");
                  $new = "INSERT INTO sell_product VALUES('$_SESSION[id]','$result[seller_id]','$p_id','$total',CURDATE(),'$qty')";
              ...
              ?><script>window.location="fa_profile.php";</script><?php
          }?>
      </div>
      ...
      <div class="row">
        ...
        <?php
           if(isset($_POST["add"])){
			  ...
			  $tm=md5(time());
			 $fnm=$_FILES["img"]["name"];
			 $dst="./fa_images/".$tm.$fnm;
			 $dst1="fa_images/".$tm.$fnm;	 
			 move_uploaded_file($_FILES["img"]["tmp_name"],$dst);
         mysqli_query($link,"INSERT INTO crop VALUES('','$breed','$name','$quantity','$price','$dst1','$disc',CURDATE(),'$id')"); 
      ...
      <div class="row">
        <div class="col-sm-12">
          <?php
              $qr = "SELECT * FROM sell_product WHERE Farmer_id='$_SESSION[id]'";
              $rel = mysqli_query($link,$qr);
              $result = mysqli_fetch_array($rel);
              if($result){
